...
class EnvironmentSyncService(private val project: Project) {

    private val environmentService: EnvironmentService = EnvironmentService.getInstance(project)

    companion object : IdeaLog {
        fun getInstance(project: Project): EnvironmentSyncService = project.service()
    }
     ...
     */
    fun showSyncDialogAndExecute(
        mode: EnvironmentSyncDialog.SyncMode,
        onPullComplete: (() -> Unit)? = null
    ) {
        ...
        val title = when (mode) {
            EnvironmentSyncDialog.SyncMode.PUSH -> "Pushing environments to Postman"
            EnvironmentSyncDialog.SyncMode.PULL -> "Pulling environments from Postman"
        }
        ...
        ProgressManager.getInstance().run(object : Task.Backgroundable(project, title) {
            override fun run(indicator: ProgressIndicator) {
                ...
                if (mode == EnvironmentSyncDialog.SyncMode.PULL && result.succeeded.isNotEmpty()) {
    ...
    private fun showSyncNotification(
        ...
        val direction = when (mode) {
            EnvironmentSyncDialog.SyncMode.PUSH -> "pushed"
            EnvironmentSyncDialog.SyncMode.PULL -> "pulled"
        }
