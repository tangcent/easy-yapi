 ...
 *
 * Handles PSI element resolution for Kotlin classes by delegating
 * to [JavaPsiAdapter] for most operations. Kotlin PSI classes
 * are compatible with Java PSI APIs for most use cases.
 *
 * This adapter is only loaded when the Kotlin plugin is available.
 *
 * @see PsiLanguageAdapter for the interface
 * @see JavaPsiAdapter for the delegate implementation
 */
@ConditionOnClass("org.jetbrains.kotlin.idea.KotlinLanguage")
class KotlinPsiAdapter : PsiLanguageAdapter {
    ...
    override fun supportsElement(element: PsiElement): Boolean {
        ...
        return id.equals("kotlin", true) || id.equals("Kotlin", true)
    }
