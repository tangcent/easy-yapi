...
import com.itangcent.easyapi.core.psi.helper.BlockingAnnotationHelper
import com.itangcent.easyapi.core.psi.helper.BlockingDocHelper
 ...
 */
open class ScriptItContext(protected val context: RuleContext) : ItContext {
    ...
    override fun psi(): PsiElement? = context.element

    override fun getName(): String? = readSync {
        (context.element as? PsiNamedElement)?.name
    }

    override fun name(): String = getName() ?: ""

    override fun doc(): String? {
        val el = context.element ?: return null
        return docHelper?.getAttrOfDocComment(el)
    }

    override fun doc(tag: String): String? = docs(tag)?.firstOrNull()

    override fun docs(tag: String): List<String>? {
        val el = context.element ?: return null
        return docHelper?.findDocsByTag(el, tag)
    }

    override fun hasDoc(tag: String): Boolean = docs(tag) != null

    override fun doc(tag: String, subTag: String): String? {
    ...
    override fun hasAnn(name: String): Boolean {
        val el = context.element ?: return false
        return annotationHelper?.hasAnn(el, name) ?: false
    }

    override fun ann(name: String): String? = ann(name, "value")

    override fun ann(name: String, attr: String): String? {
        val el = context.element ?: return null
        return annotationHelper?.findAttrAsString(el, name, attr)
    }

    override fun annValue(name: String): Any? = annValue(name, "value")

    override fun annValue(name: String, attr: String): Any? {
        val el = context.element ?: return null
        return annotationHelper?.findAttr(el, name, attr)
    }

    override fun annMap(name: String): Map<String, Any?>? {
        val el = context.element ?: return null
        return annotationHelper?.findAnnMap(el, name)
    }

    override fun annMaps(name: String): List<Map<String, Any?>>? {
        val el = context.element ?: return null
        return annotationHelper?.findAnnMaps(el, name)
    }

    override open fun sourceCode(): String? = readSync { context.element?.text }

    override open fun defineCode(): String? {
    ...
    override open fun contextType(): String = "unknown"
     ...
     */
    override open fun canonicalText(): String = name()

    /**
     * Get an extension value from the rule context.
     * Used by scripts to access context-specific data (e.g. request, response).
     */
    override fun getExt(name: String): Any? = context.getExt(name)

    /**
     * Set an extension value on the rule context.
     */
    override fun setExt(name: String, value: Any?) = context.setExt(name, value)
