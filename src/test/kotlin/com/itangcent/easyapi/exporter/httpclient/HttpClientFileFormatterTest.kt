...
class HttpClientFileFormatterTest {
    ...
    @Test
    fun testFormatWithDuplicateContentTypeInHeaders() {
        val endpoints = listOf(
            ApiEndpoint(
                name = "Create User",
                metadata = HttpMetadata(
        ...
        val contentTypeCount = result.split("Content-Type:").size - 1
        assertEquals("Content-Type should appear only once", 1, contentTypeCount)
        assertTrue(result.contains("Content-Type: application/json"))
        assertTrue(result.contains("Authorization: Bearer token123"))
    }

    @Test
    fun testFormatWithContentTypeInHeadersOnly() {
        val endpoints = listOf(
            ApiEndpoint(
                name = "Create User",
                metadata = HttpMetadata(
    ...
    @Test
    fun testFormatWithNoContentTypeInHeaders() {
        val endpoints = listOf(
            ApiEndpoint(
                name = "Get User",
                metadata = HttpMetadata(
    ...
    @Test
    fun testFormatWithContentTypeCaseInsensitive() {
        val endpoints = listOf(
            ApiEndpoint(
                name = "Create User",
                metadata = HttpMetadata(
    ...
    @Test
    fun testFormatAllWithMultipleEndpoints() {
        val endpoints = listOf(
            ApiEndpoint(
                name = "Get User",
        ...
        val result = HttpClientFileFormatter.formatAll(endpoints, "http://localhost:8080", "test-api")

        assertTrue(result.contains("### test-api"))
        assertTrue(result.contains("### Generated by EasyAPI"))
        assertTrue(result.contains("### Total: 2 endpoints"))
        assertTrue(result.contains("GET http://localhost:8080/api/users"))
        assertTrue(result.contains("POST http://localhost:8080/api/users"))
    }

    @Test
    fun testFormatSingleEndpoint() {
        ...
        val result = HttpClientFileFormatter.format(endpoint, "http://localhost:8080")

        assertTrue(result.contains("### Get User"))
        assertTrue(result.contains("# Retrieve user by ID"))
        assertTrue(result.contains("GET http://localhost:8080/api/users/1"))
    }
    ...
    fun testFormatWithEmptyHost() {
        ...
        val result = HttpClientFileFormatter.format(endpoints, "")

        assertTrue(result.contains("GET /api/users"))
    }

    @Test
    fun testFormatWithQueryParamsWithValues() {
        val endpoints = listOf(
            ApiEndpoint(
                name = "List Users",
                metadata = HttpMetadata(
