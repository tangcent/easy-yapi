...
/**
 * Integration test for the `field-order-alphabetically` extension.
 *
 * The extension defines a rule for [RuleKeys.FIELD_ORDER_WITH]:
 *   - `field.order.with=groovy: return a.name().compareTo(b.name())`
 *
 * The comparator returns the result of `a.name().compareTo(b.name())`, producing
 * ascending alphabetical order (A-Z).
 */
class FieldOrderAlphabeticallyConfigIntegrationTest : EasyApiLightCodeInsightFixtureTestCase() {
    ...
    override fun createConfigReader(): TestConfigReader {
        val extension = ExtensionConfigRegistry.getExtension("field-order-alphabetically")
        assertNotNull("field-order-alphabetically extension should exist", extension)
        return TestConfigReader.fromConfigText(project, extension?.content ?: "")
    }

    fun testFieldOrderAlphabeticallyConfigLoadsCorrectly() = runTest {
        val extension = ExtensionConfigRegistry.getExtension("field-order-alphabetically")
        assertNotNull(extension)
        assertEquals("field-order-alphabetically", extension?.code)
        assertTrue(extension?.content?.isNotBlank() == true)
    }

    /**
     * The core rule: comparing "apple" (a) against "zebra" (b) should resolve
     * [RuleKeys.FIELD_ORDER_WITH] to a negative value (apple comes first).
     */
    fun testFieldOrderWithRuleAppleBeforeZebra() = runTest {
        ...
        assertTrue(
            "apple should come before zebra (cmp < 0). Was: $cmp",
            cmp!! < 0
        )
    }
     ...
     * [RuleKeys.FIELD_ORDER_WITH] to a positive value (zebra comes after).
     */
    fun testFieldOrderWithRuleZebraAfterApple() = runTest {
        val psiClass = findClass("com.itangcent.fieldorder.OrderDTO")
        assertNotNull("Should find OrderDTO", psiClass)
        ...
        assertNotNull("FIELD_ORDER_WITH should resolve to an int. Was: $result", cmp)
        assertTrue(
            "zebra should come after apple (cmp > 0). Was: $cmp",
            cmp!! > 0
        )
    }
    ...
    fun testFieldsOrderedAlphabetically() = runTest {
        val psiClass = findClass("com.itangcent.fieldorder.OrderDTO")
        assertNotNull("Should find OrderDTO", psiClass)
        ...
        // Expected alphabetical order: apple, banana, mango, zebra
        assertEquals("First field should be 'apple'", "apple", fieldNames[0])
        assertEquals("Second field should be 'banana'", "banana", fieldNames[1])
        assertEquals("Third field should be 'mango'", "mango", fieldNames[2])
        assertEquals("Fourth field should be 'zebra'", "zebra", fieldNames[3])
    }
}
