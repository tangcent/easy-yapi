...
class GrpcMethodResolverTest : EasyApiLightCodeInsightFixtureTestCase() {
    ...
    private fun loadGeneratedGrpcFiles() {
        ...
        val generatedGrpcDir = File("build/generated/source/proto/test/grpc/com/itangcent/easyapi/core/grpc/test")
        if (generatedGrpcDir.exists()) {
            generatedGrpcDir.listFiles()?.forEach { file ->
                if (file.name.endsWith(".java")) {
                    val content = file.readText()
                    val path = "com/itangcent/easyapi/core/grpc/test/${file.name}"
                    loadFile(path, content)
                }
            }
        }

        val generatedJavaDir = File("build/generated/source/proto/test/java/com/itangcent/easyapi/core/grpc/test")
    ...
    fun testResolveFromRpcMethodAnnotation() = runTest {
        val psiClass = findClass("com.itangcent.easyapi.core.grpc.test.EchoServiceGrpc")
    ...
    fun testExtractTypesFromAnnotation() = runTest {
        ...
        assertNotNull("Response type should be resolved", echoMethod.responseType)
        assertEquals("Request type should be EchoRequest", 
            "com.itangcent.easyapi.core.grpc.test.EchoRequest", echoMethod.requestType!!.qualifiedName)
        assertEquals("Response type should be EchoResponse", 
            "com.itangcent.easyapi.core.grpc.test.EchoResponse", echoMethod.responseType!!.qualifiedName)
    }
