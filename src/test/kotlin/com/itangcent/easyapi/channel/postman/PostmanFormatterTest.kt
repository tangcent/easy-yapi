...
class TestablePostmanFormatter {
    ...
    fun testBuildBody(endpoint: ApiEndpoint): PostmanBody? {
        ...
        if (contentType.contains("json")) {
            val raw = when {
                httpMeta?.body != null -> com.itangcent.easyapi.core.util.json.GsonUtils.prettyJson(httpMeta.body)
                bodyParams.isNotEmpty() -> bodyParams.associate { it.name to (it.example ?: it.defaultValue ?: "") }
                    .let { com.itangcent.easyapi.core.util.json.GsonUtils.prettyJson(it) }
                else -> "{}"
            }
            return PostmanBody(
                mode = "raw",
        ...
        if (httpMeta?.body != null) {
            return PostmanBody(
                mode = "raw",
                raw = com.itangcent.easyapi.core.util.json.GsonUtils.prettyJson(httpMeta.body),
                options = mapOf("raw" to mapOf("language" to "json"))
            )
        }
