...
class PostmanCollectionHelperTest {
    ...
    private lateinit var settings: PostmanSettings
...
object PostmanCollectionHelperTestHelper {
    fun getCollectionId(settings: PostmanSettings, module: String): String? {
        val properties = java.util.Properties()
        settings.postmanCollections?.byteInputStream()?.let { properties.load(it) }
        return properties.getProperty(module)
    }

    fun setCollectionId(settings: PostmanSettings, module: String, collectionId: String) {
        val properties = java.util.Properties()
        settings.postmanCollections?.byteInputStream()?.let { properties.load(it) }
    ...
    fun removeCollectionId(settings: PostmanSettings, module: String) {
        val properties = java.util.Properties()
        settings.postmanCollections?.byteInputStream()?.let { properties.load(it) }
    ...
    fun readCollections(settings: PostmanSettings): Map<String, String> {
        val properties = java.util.Properties()
        settings.postmanCollections?.byteInputStream()?.let { properties.load(it) }
