...
interface TemplateExpression {
    fun eval(request: Request, stringBuilder: StringBuilder)
    ...
    companion object {
        ...
        internal fun parseTemplate(template: String): TemplateExpression {
            ...
            // Match all patterns: ${...}, $xx, $xx.xx, ${if ...}, and ${end}
            ...
            regex.findAll(template).forEach {
                ...
                val scriptMatch = it.groupValues[1]
                val simpleMatch = it.groupValues[2]
...
class PlainText(
    ...
    override fun eval(request: Request, stringBuilder: StringBuilder) {
...
class RequestProperty(
    ...
    override fun eval(request: Request, stringBuilder: StringBuilder) {
        val value = request.getPropertyValue(property)
...
class ResponseProperty(
    ...
    override fun eval(request: Request, stringBuilder: StringBuilder) {
        val value = request.response?.firstOrNull()?.getPropertyValue(property)
        value?.let { stringBuilder.append(it) }
    }
}
...
class CompositeTemplateExpression(
    ...
    override fun eval(request: Request, stringBuilder: StringBuilder) {
        expressions.forEach { it.eval(request, stringBuilder) }
    }
}
...
private class ScriptEngineHelper {
    ...
    fun eval(script: String, request: Request): Any? {
        ...
        getAllFields(request.javaClass).forEach { field ->
...
class ScriptTemplateExpression(
    ...
    override fun eval(request: Request, stringBuilder: StringBuilder) {
        val result = scriptHelper.eval(script, request) ?: script
        when (result) {
            is String -> stringBuilder.append(result)
            is TemplateExpression -> result.eval(request, stringBuilder)
...
class MDObjectExpression(
    ...
    override fun eval(request: Request, stringBuilder: StringBuilder) {
        ...
        val customObjectWriter = CustomObjectWriter(writer, titles.map { it.key to it.value })
...
class SinglePropertyCondition(
    ...
    override fun eval(request: Request): Boolean {
        val value = request.getPropertyValue(property)
...
class ScriptCondition(
    ...
    override fun eval(request: Request): Boolean {
        val result = scriptHelper.eval(script, request) ?: false
...
class ConditionalExpression(
    ...
    override fun eval(request: Request, stringBuilder: StringBuilder) {
        if (condition.eval(request)) {
            expression.eval(request, stringBuilder)
        }
    }
}
